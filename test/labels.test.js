...
test('라벨에 설정한 기간 숫자가 그대로 들어간다', () => {
  assert.equal(signalLabel('golden', periods).brief, '50일선 위로');
  assert.equal(signalLabel('golden', periods).full, '50일 이동평균선이 200일 이동평균선을 아래에서 위로 뚫었습니다.');
});

test('기간을 바꾸면 라벨도 따라간다', () => {
  assert.equal(signalLabel('dead', { short: 20, long: 60 }).brief, '20일선 아래로');
});
...
test('표시용 이름은 이모지를 앞에 붙인다', () => {
  assert.equal(signalName('proximity', periods), '🟡 50일선 근접');
});
