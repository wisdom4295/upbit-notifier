...
| **일요일 밤 10시** 이후 첫 실행 | 이번 주(월~일) 같은 내용 + **신호가 맞았나** 집계 |
...
주간 리포트는 여기에 한 칸이 더 붙습니다.

```
■ 신호가 맞았나 (하루 뒤 기준)
🟢 50선 위로  4건 중 3건 맞음  평균 ▲2.15%
🔴 50선 아래로  2건 중 2건 맞음  평균 ▼2.1%
🟡 50선 근접  3건  평균 ▲1.03%
(1건은 아직 하루가 안 지나 집계에서 뺐습니다)

맞음 = 🟢는 하루 뒤 올랐고, 🔴는 내린 경우입니다.
```
...
| 칸 | 무엇을 보나 | 어디에 |
| --- | --- | --- |
| **움직임** | 감시 중인 코인이 얼마나 올랐고 내렸는지, 그 기간의 고점과 저점. 알림이 한 건도 없던 코인도 나옵니다 | 둘 다 |
| **지금 두 선** | 지금 두 선이 몇 % 벌어져 있는지. **교차에 가까운 순서**로 위에서부터. `좁혀지는 중`이면 다음에 볼 종목입니다 | 둘 다 |
| **받은 알림** | 언제 어느 코인에 무엇이 왔는지와 그 뒤 1시간·4시간·하루 변화 (주간은 날짜까지) | 둘 다 |
| **신호가 맞았나** | 온 알림이 가리킨 방향으로 실제로 움직였는지. 🟢는 올랐으면, 🔴는 내렸으면 맞은 것으로 셉니다 | **주간만** |

적중 집계를 하루치에서 뺀 이유는 표본이 너무 작기 때문입니다. 하루에 오는 알림은 보통 한두 건이라
`1건 중 1건 맞음`이 100%처럼 읽히고, 건별 성과는 바로 아래 알림 목록에 이미 적혀 있습니다.
한 주치쯤 모여야 "이 조건이 나한테 맞나"를 따질 수 있습니다.

**성과는 알림이 온 시점의 가격과 비교한 값**입니다. 아직 그 시간이 지나지 않았으면 `—`로 적고,
하루가 지나지 않은 건은 "맞았나" 집계에서 빼고 몇 건을 뺐는지 밝힙니다. 한 건이 크게 튀면 평균이 왜곡되므로
평균과 함께 **맞은 횟수**를 같이 셉니다.
...
src/report-stats.js         기간 등락·두 선 간격·신호 적중 집계
...
.github/workflows/          watch(1분 주기 + 리포트) · test
```
