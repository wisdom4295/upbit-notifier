...
- **회고 대시보드 PWA** — 오늘/이번 주에 뜬 신호와 그 이후 성과, 내 매매 기록을 한 화면에서 (GitHub Pages)
...
| **오늘** | 오늘(KST 0시~) 뜬 신호와 그 이후 성과, 오늘의 매매 |
| **이번 주** | 이번 주(월요일~) 기준 같은 내용 |

회고 탭에서 보는 것:

- **신호 이력** — 언제 어떤 코인에서 무슨 신호가 떴는지, 당시 가격, 그리고 **신호 후 1시간 / 4시간 / 24시간
  가격이 어떻게 움직였는지**. "이 신호가 쓸만했나"를 판단하는 근거입니다.
- **보유 현황** — 내 매매 기록으로 계산한 코인별 평단·수량과 현재가 기준 평가손익
- **매매 기록** — 코인·매수/매도·가격·수량·메모를 직접 입력. 기간 실현손익이 자동으로 집계됩니다

신호 이력은 알림이 나갈 때 워크플로가 `history/YYYY-MM.json`에 쌓고 커밋합니다. 대시보드는 그 파일을
그대로 읽으므로 **서버도 DB도 필요 없습니다.** 알림이 한 번도 안 나갔다면 이력이 비어 있는 게 정상입니다.

> **매매 기록은 이 브라우저에만 저장됩니다** (`localStorage`). 서버가 없으니 다른 기기와 동기화되지 않고,
> 브라우저 데이터를 지우면 함께 지워집니다. 신호 이력은 레포에 쌓이므로 기기와 무관합니다.
>
> 손익은 **이동평균법**으로 계산합니다. 매수하면 평단이 갱신되고, 매도하면 `(매도가 - 평단) × 수량`이
> 실현손익으로 잡힙니다. 세금·수수료는 반영하지 않습니다.
